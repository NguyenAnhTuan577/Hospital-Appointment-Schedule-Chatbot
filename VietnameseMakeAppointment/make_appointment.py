...
xxx={
  ...
  "currentIntent": {
    ...
    "slots": {
      ...
      "DiseaseOne": None,
      "DiseaseTwo": None,
      "Doctor": 'Trương Hoàng Vĩnh Khiêm',
      "Name": None,
      "PhoneNumber": None,
      "Confirmation": None,
      "Speciality": 'Khoa Chấn Thương Chỉnh Hình',
      "Time": None,
      "FormattedDate": "1"
    }
  },
...
def build_options(slot, speciality, doctor, date, time):
    """
    ...
    if slot == 'Speciality':
        try:
            connection = psycopg2.connect("dbname='qjunivvc' user='qjunivvc' host='arjuna.db.elephantsql.com' password='qcGs166MeIBq6DTtdOqCOs7l_lIJhcLL'")
        ...
        try:
            ...
            if speciality:  
                if not doctor:
                    cursor.execute("SELECT * FROM doctors as d,medical_specialities as ms WHERE ms.id=d.speciality_id and ms.name = '{}';".format(speciality))
                else:
                    cursor.execute("SELECT * FROM doctors as d,medical_specialities as ms WHERE ms.id=d.speciality_id and ms.name = '{}' and d.name ILIKE '%{}';".format(speciality,doctor))
            else:
                if doctor:
                    cursor.execute("SELECT * FROM doctors as d,medical_specialities as ms WHERE ms.id=d.speciality_id and d.name ILIKE '%{}';".format(doctor))
            records = cursor.fetchall()
        except (Exception, psycopg2.Error) as error :
             print ("Error while connecting to PostgreSQL", error)
        finally:
    ...
    #     return res
        ...
        try:
            ...
            # Print PostgreSQL version
            if speciality:
                cursor.execute("SELECT distinct wh.day FROM working_hours as wh, doctors as d , medical_specialities as ms where wh.doctor_id=d.id and ms.id=d.speciality_id and d.name = '{}' and ms.name = '{}';".format(doctor,speciality))
            else:
                cursor.execute("SELECT distinct wh.day FROM working_hours as wh, doctors as d , medical_specialities as ms where wh.doctor_id=d.id and ms.id=d.speciality_id and d.name = '{}';".format(doctor))
            records = cursor.fetchall()
            if len(records)==0:
                return None
        except (Exception, psycopg2.Error) as error :
        ...
        try:
            ...
            date_weekday=day_strings[datetime.datetime.strptime(date, '%Y-%m-%d').date().weekday()] #ngày kiểu datetime
             # Print PostgreSQL version
            if speciality:
                cursor.execute("SELECT wh.time FROM working_hours as wh, doctors as d , medical_specialities as ms where wh.doctor_id=d.id and ms.id=d.speciality_id and d.name = '{}' and ms.name = '{}' and wh.day='{}'  order by wh.time;".format(doctor,speciality,date_weekday))
            else:
                cursor.execute("SELECT wh.time FROM working_hours as wh, doctors as d , medical_specialities as ms where wh.doctor_id=d.id and ms.id=d.speciality_id and d.name = '{}'  and wh.day='{}'  order by wh.time;".format(doctor,date_weekday))
...
def make_appointment(intent_request):
    ...
    URL = "https://graph.facebook.com/{}?fields=name,first_name,last_name&access_token=EAACtffPGBe4BAGvNefOdwJDnB8s2wZAQgNlEBJZCfxgbdZC8ktRuB3NGTIPBR47QDc5KDwa9w3osAbZAnpWQNQneMr4v4SbBauZAjgx06x1xZCZA2dSPFV1rBa1dhnkRcrSM8sgKL5ZAtM20Ww3mnD11jWYweE41x5a8HjkDISWyUFF2TyzhORj5".format(psid)
        ...
        if not speciality and not doctor:
            return elicit_slot(
                ...
                build_response_card(
                    ...
                    build_options('Speciality', speciality,doctor,date,time)))
        # không bác sĩ và có khoa
        elif not doctor:
            if build_options('Doctor', speciality,doctor,date,time)==None:
                ...
                build_response_card(
                    ...
                    'Mời bạn chọn bác sĩ',
                    build_options('Doctor', speciality,doctor,date,time)))
        #có bác sĩ
        else:
            ...
            if len(element_of_name)==1:
                try:
                    ...
                    if not speciality:
                        cursor.execute("SELECT * FROM doctors as d,medical_specialities as ms WHERE ms.id=d.speciality_id and  d.name ILIKE '% {}';".format(doctor))
                        message='Có nhiều bác sĩ tên {}, bạn muốn khám với bác sĩ nào ạ?'.format(doctor)
                    else:
                        cursor.execute("SELECT * FROM doctors as d,medical_specialities as ms WHERE ms.id=d.speciality_id and ms.name = '{}' and d.name ILIKE '% {}';".format(speciality,doctor))
                        message='Có nhiều bác sĩ tên {} tại khoa {}, bạn muốn khám với bác sĩ nào ạ?'.format(doctor,speciality)
                    records = cursor.fetchall()
                    if len(records)==1:
                        doctor=records[0][1]
                        slots['Doctor']=doctor    
                ...
                if not speciality:
                    try:
                        ...
                        # Print PostgreSQL Connection properties
                        print ( connection.get_dsn_parameters(),"\n")
                        cursor.execute("SELECT ms.name FROM doctors as d,medical_specialities as ms WHERE ms.id=d.speciality_id and d.name = '{}';".format(doctor))
                        records = cursor.fetchall()
                        if len(records)==1:
            ...
            if not date:
                if speciality:
                    if build_options('Date', speciality,doctor,date,time)==None:
                        slots['Doctor']=None
                        doctor=None
                        return elicit_slot(
                        output_session_attributes,
                        ...
                        build_response_card(
                            ...
                            build_options('Date', speciality,doctor,date,time)))
                else:
                    return elicit_slot(
                        output_session_attributes,
                        intent_request['currentIntent']['name'],
                ...
                #slots['Date']=date
                ################## end fix bug
                if build_options('Time', speciality,doctor,date,time)==None:
                    slots['Date']=None
                    date=None
                    return elicit_slot(
                    output_session_attributes,
                    ...
                    build_response_card(
                        'Thời gian làm việc của bác sĩ {} trong ngày {}'.format(doctor,date_display),
                        'Mời bạn chọn thời gian hẹn bác sĩ',
                        build_options('Time', speciality,doctor,date,time)))
                ...
                try:
                    ...
                    record_to_insert = (doctor,speciality,date,time,name,DateOfBird,PhoneNumber,psid)
    ...
    return close(
        ...
        {
            'contentType': 'PlainText',
            'content': 'Thông tin chi tiết của bạn như sau: Bệnh nhân {} sinh ngày {} có số điện thoại {} có lịch hẹn với bác sĩ {} của khoa {} vào lúc {} ngày {}'.format(name,DateOfBird,PhoneNumber, doctor,speciality, time, date)
        }
    )
